...
NSString * WidgetDemo$ModuleRequireFormat = @"(function(exports){"
...
@implementation WidgetDemoObject
...
{
	// create WidgetDemo global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "WidgetDemo" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_widgetdemo = [[WidgetDemoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_widgetdemo];
	
	NSString *_widgetdemoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _widgetdemoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_widgetdemo valueForKey:name];
...
{
	NSString *js = [[NSString alloc] initWithFormat:WidgetDemo$ModuleRequireFormat,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.widgetdemo.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.widgetdemo.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
