...
//Rare exceptions to indicate a bug in the _widgetdemo code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_widgetdemo = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_widgetdemo]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.widgetdemo.module"
